...
adverselist = Select(title="Adverse effect:", value = 'Drug ineffective',
                     options=open('data/adverselist.txt').read().split('\n'))
...
cast = TextInput(title="Adverse effect:")
...
# Load data
ML_data = pd.read_csv('data/headache_READY.csv')
originaladverseinitlist = pd.read_csv('data/adverselistinit.csv') # initialization for helping make prediction based on user selection dropdown 
drugcode = pd.read_csv('data/drugnamecodes.csv') # drug code numbers
Model_data = pd.DataFrame(ML_data[['weight','age','sex','drug','Drug ineffective','Incorrect drug administration duration','Dyspnoea','Headache','Nausea','Dizziness','Extra dose administered','Diarrhoea','Pneumonia','Pain','Rash','Arthralgia','Myocardial infarction','Off label use','Fatigue','Malaise','Fall','Death','Drug effect incomplete','Type 2 diabetes mellitus','Expired product administered','Pyrexia','Intentional product misuse','Abdominal pain upper','Gastrointestinal haemorrhage','Vomiting','Pruritus','Blood pressure increased','Pulmonary embolism','Pain in extremity','Abdominal discomfort','Depression','Urticaria','Somnolence','Insomnia','Atrial fibrillation','Weight decreased','Muscle spasms','Incorrect dose administered','Deep vein thrombosis','Weight increased','Asthenia','Cough','Cerebrovascular accident','Hypersensitivity','Drug hypersensitivity','Feeling abnormal','Myalgia','Nasopharyngitis','Renal failure acute','Poor quality drug administered','Chest pain','Back pain','Overdose','Loss of consciousness','Dyspepsia','Constipation','Anaemia','Product use issue','Tremor','Hypotension','Inappropriate schedule of drug administration','Peripheral swelling','Convulsion','Infusion related reaction','Syncope','Hypertension','Drug dose omission','Abdominal pain','Cardiac disorder','Throat irritation','Joint swelling','Contusion','Blood glucose increased','Haemorrhage','Chest discomfort','Injection site pain','Dehydration','Anxiety','Swelling face','Alopecia','Wrong technique in drug usage process','Sepsis','Foreign body','Muscular weakness','Drug effect decreased','Epistaxis','Arthritis','Haemoglobin decreased','Burning sensation','Medication error','Product quality issue','Palpitations','Erythema','Chronic obstructive pulmonary disease','Condition aggravated','Accidental exposure to product by child','Urinary tract infection','Vision blurred','Cellulitis']])
...
t_v='Based on <i>3192</i> adverse effect reports submitted to the FDA involving <i>'
t_v=t_v+dname.value + '</i>, and given that you are <i>Female</i>, <i>'+str(max_year.value)+'</i> years old, '
t_v=t_v+'and weigh <i>'+str(oscars.value)+'</i> kg, in the event of <i>'+adverselist.value.lower()+'</i> reaction, '
t_v=t_v+'the probability that such reaction is serious (e.g. resulting in hospitalization) is <b>7%</b>. '
t_v=t_v+'This prediction is based on machine learning logistic regression and random forest classifier</i> using <a href="https://en.wikipedia.org/wiki/Scikit-learn" target="_blank">scikit</a>. '
t_v=t_v+'Overall predictive acuracy of the model is <i>72.5%</i>.'

ml_ = Div(text=t_v, width=580, height=100)

predict_button = Button(label='Predict')

wb = widgetbox([ml_,predict_button]) #l_ = layout([[desc],[ml_, predict_button]])

Mll = layout([[wb],[adverselist,mlplt]])
...
def up_tab():
    ...
    #print str(adverselist.ref)
    #print str(adverselist.trigger)
    #print str(adverselist._visit_value_and_its_immediate_references)
    ...
    output.append(oscars.value) # weight
    output.append(max_year.value) # age
    output.append(radio_group.active+1) # sex
    #drug codes: acetaminophen 0 advil 1 aleve 2 aspirin 3 excedrin 4 ibuprofen 5 tylenol 6 vitamin 7    
    try:    
        drugnamecodeselected = drugcode[dname.value.lower()]
    except:
        # tylenol is the default option for those drugs not coded yet
        drugnamecodeselected = 6
    
    print 'dname.value: ' + dname.value
    print 'drugnamecodeselected: ' + str(drugnamecodeselected)
    
    output.append(drugnamecodeselected) # drug
    
    #azeros = np.zeros(shape=(1,100))
    adverseinitlist = originaladverseinitlist.copy()
    adverseinitlist[adverselist.value] = 1
    temp=[]
    for row in adverseinitlist.iterrows():
        index, data = row
    ...
    datapoint = neighR.predict_proba(output)[:,0][0]
    # Deal with special case: death
    if (adverselist.value == 'Death'):
        datapoint = 1
    ...
    new_text=new_text+'and weigh <i>'+str(oscars.value)+'</i> kg, in the event of <i>'+adverselist.value.lower()+'</i> reaction, '
    new_text=new_text+'the probability that such reaction is serious (e.g. resulting in hospitalization) is <b>'+str(s_datapoint)+'</b>. '
    new_text=new_text+'This prediction is based on machine learning logistic regression and random forest classifier</i> using <a href="https://en.wikipedia.org/wiki/Scikit-learn" target="_blank">scikit</a>. '
...
Robert Schroll [1:31 PM]  
Why is the important info in the last tab?  And why is that tab labelled "ML"?  (I know, but your audience won't.)

[1:32]  
But that info still isn't really what I want. That tells me the probability of a particular adverse reaction  to a particular drug being serious.  I want to know which is the least risky drug to use.

[1:33]  
I definitely want all the drugs on a single page.  I probably want all reactions combined (at first, at least).  I'd like actual reaction rates, not just conditional probabilities, but you may not have that info.

[1:34]  
And I want that info graphically.  Right now, my eye is drawn to the ROC curve, which is static and not telling me anything interesting.  (It should go on an about page.)

Roger Stanev [1:35 PM]  
many thanks Rob, great feedback

Robert Schroll [1:37 PM]  
Try to refocus to the user experience.  The more exploratory stuff would be nice on an about page, for people who want more info.

Roger Stanev [1:41 PM]  
For sure. what you're suggesting makes perfect sense. to be continued.
'''